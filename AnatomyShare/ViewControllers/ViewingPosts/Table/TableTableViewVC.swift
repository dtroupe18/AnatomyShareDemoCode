...
class TableTableViewVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    // Flag to determine when posts are being loaded
    ...
    // oldest key for whatever posts we are loading
    var oldestKey: String!

    // Flags to determine what posts are loaded
    var showingAllPosts: Bool = false
    var showingCategoryPosts: Bool = false
    var showingRegionPosts: Bool = false
    var showingCategoryRegion: Bool = false
    
    // variables needed to query additional posts
    ...
    override func viewDidLoad() {
        ...
        // Reload for filter
        NotificationCenter.default.addObserver(self, selector: #selector(self.reloadTableView), name: NSNotification.Name(rawValue: "reloadTableTableView"), object: nil)
    }
        ...
        if passedData.postDict[key] != nil {
            ...
            cell.editTapAction = { (BetterPostCell) in
                if indexPath.row + 1 <= self.postKeys.count {
                    ...
                    DispatchQueue.main.async {
                        self.performSegue(withIdentifier: "toEditFromTableTable", sender: nil)
                    }
                }
            }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row + 1 >= self.postKeys.count && !self.isLoading {
            if self.showingAllPosts {
                if let oldest = self.oldestKey, let last = postKeys.last {
                    if last == oldest {
                        return
                    }
                    self.fetchMoreTablePosts(tableNumber: self.tableNumber, lastVisibleKey: last, { (done) in
                        if done {
            ...
            else if self.showingRegionPosts {
                if let oldest = self.oldestKey, let last = postKeys.last, let reg = self.region {
                    if last == oldest {
                        return
                    }
                    self.fetchMoreRegionPosts(tableNumber: self.tableNumber, region: reg, lastVisibleKey: last, { (done) in
                        if done {
                            Timer.scheduledTimer(withTimeInterval: 0.75, repeats: false, block: { (timer) in
                                self.tableView.reloadData()
                                self.isLoading = false
            ...
            else if self.showingCategoryPosts {
                if let oldest = self.oldestKey, let last = postKeys.last, let cat = self.category {
                    if last == oldest {
                        return
                    }
                    self.fetchMoreCategoryPosts(tableNumber: self.tableNumber, category: cat, lastVisibleKey: last, { (done) in
                        if done {
                            Timer.scheduledTimer(withTimeInterval: 0.75, repeats: false, block: { (timer) in
                                self.tableView.reloadData()
                                self.isLoading = false
            ...
            else if self.showingCategoryRegion {
                if let oldest = self.oldestKey, let last = postKeys.last, let cat = self.category, let reg = self.region {
                    if last == oldest {
                        return
                    }
                    self.fetchMoreCategoryRegionPosts(tableNumber: self.tableNumber, category: cat, region: reg, lastVisibleKey: last, { (done) in
                        if done {
                            Timer.scheduledTimer(withTimeInterval: 0.75, repeats: false, block: { (timer) in
                                self.tableView.reloadData()
                                self.isLoading = false
    ...
    @objc func segueToScrollView(_ sender: UIPinchGestureRecognizer) {
        if sender.state == .began {
            ...
            if let indexPath = tableView.indexPathForRow(at: touch) {
                ...
                DispatchQueue.main.async {
                    self.performSegue(withIdentifier: "toZoomFromTableTable", sender: sender)
                }
            }
        }
    }
    ...
    @objc func viewAllCommentsButtonPressed(_ sender: UIButton!) {
        ...
        DispatchQueue.main.async {
            self.performSegue(withIdentifier: "toCommentsFromTableTable", sender: sender)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toZoomFromTableTable" {
        ...
        if segue.identifier == "toEditFromTableTable" {
            if let destination = segue.destination as? EditPostViewController {
                if let index = selectedRow {
                    ...
                    if let post = passedData.postDict[postDraft.key!] {
                        ...
                        // this image should be in the cache because it is currently visible
        ...
        if segue.identifier == "toCommentsFromTableTable" {
